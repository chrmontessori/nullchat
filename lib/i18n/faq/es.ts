...
Esto es útil si alguien está mirando por encima de tu hombro o si tu pantalla es visible para otros. A simple vista, parece que estás editando un documento, no teniendo una conversación cifrada. Pulsa Shift cinco veces de nuevo para volver a la vista normal del chat.`,
  ...
  faq_32_title: "¿Puedo agregar nullchat a mi pantalla de inicio?",
  faq_32_body: `Sí. nullchat es compatible con Agregar a la pantalla de inicio tanto en iOS como en Android. En iOS Safari, toca el botón de compartir y selecciona "Agregar a la pantalla de inicio". En Android Chrome, toca el menú y selecciona "Agregar a la pantalla de inicio" o "Instalar aplicación". Esto crea un acceso directo independiente que abre nullchat sin la interfaz del navegador — sin barra de direcciones, sin pestañas. Se ve y se siente como una aplicación nativa.

Importante: nullchat deliberadamente no usa un service worker ni almacena datos en caché para uso sin conexión. No hay modo sin conexión. El acceso directo de la pantalla de inicio simplemente abre el sitio en vivo — no se almacena nada en tu dispositivo más allá del acceso directo en sí. Esta es una decisión de seguridad: almacenar en caché páginas de chat cifradas o scripts de service worker en el dispositivo crearía evidencia forense de que nullchat fue utilizado. El acceso directo no deja rastro más allá de su propio ícono, que puede eliminarse en cualquier momento.`,
};
