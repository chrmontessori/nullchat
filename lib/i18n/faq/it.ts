...
export const it: Record<FaqKey, string> = {
  ...
  faq_2_body: `Tu e la persona con cui vuoi parlare concordate in anticipo un segreto condiviso — di persona, al telefono, come preferite. Entrambi digitate quel segreto in nullchat e vi trovate nella stessa stanza crittografata. Non esiste un elenco di stanze, nessuna directory, nessun modo per navigare. Per entrare, il tuo browser deve dimostrare al server di conoscere il segreto, quindi conoscere l'identificativo di una stanza non basta da solo. Se non conosci il segreto, la stanza per te non esiste.`,
...
L'indicatore di robustezza nella schermata di ingresso ti dà un'idea approssimativa di quanto il tuo segreto sia resistente agli attacchi di forza bruta, ma nessun indicatore sostituisce il buon senso. Nel dubbio, rendilo più lungo e più casuale.`,
  ...
  faq_4_body: `Quando inserisci il tuo segreto condiviso, il tuo browser lo elabora con Argon2id, una funzione di derivazione della chiave ad alto consumo di memoria, in due derivazioni separate. Ognuna ha il proprio salt e usa 16 MiB di memoria e 3 iterazioni. Tutto questo avviene sul tuo dispositivo.

1. La prima derivazione produce l'ID stanza. Viene inviato al server, così il server sa a quale stanza connetterti. Il server non vede mai il tuo segreto effettivo.

2. La seconda derivazione produce 64 byte in un unico passaggio, che vengono divisi in due: una chiave di crittografia a 256 bit e un segreto di accesso alla stanza. La chiave di crittografia non lascia mai il tuo browser. Il segreto di accesso alla stanza è il modo in cui il tuo browser dimostra al server che conosci il segreto condiviso: per questo conoscere solo l'ID stanza non basta per entrare in una stanza.

Poiché Argon2id richiede un grande blocco di RAM per ogni tentativo, rende molto più difficile indovinare la tua password per forza bruta con GPU e ASIC rispetto alle funzioni di derivazione della chiave più vecchie.

Ogni messaggio viene crittografato con NaCl secretbox (XSalsa20-Poly1305) usando la chiave di crittografia prima di lasciare il tuo dispositivo. Il server riceve, memorizza e inoltra solo testo cifrato, privo di significato senza la chiave. Non possiamo leggere i tuoi messaggi, e nessuno può farlo a meno che non conosca il segreto condiviso.`,
  faq_5_title: "Cosa vede il server?",
  faq_5_body: `Il server vede:
• Un hash derivato da Argon2id (l'ID stanza) — non la tua password
• Un valore derivato dal tuo segreto condiviso con Argon2id che dimostra che lo conosci. Il server ne conserva solo un hash, e questo valore non rivela né il segreto né la chiave di crittografia.
...
Un alias è un'etichetta, non un'identità verificata. Chiunque conosca il segreto condiviso può entrare nella stanza e impostare il proprio alias su qualsiasi valore, quindi considera chiunque si trovi in una stanza come una persona che possiede il segreto. Se devi essere sicuro di chi sia il tuo interlocutore, verificalo fuori banda, ad esempio concordando in anticipo una parola in codice. Condividi un segreto solo con persone di cui ti fidi.`,
  faq_14_title: "C'è un limite di partecipanti?",
  faq_14_body: `Ogni stanza supporta fino a 50 connessioni simultanee. Se la stanza è piena, vedrai un messaggio "La stanza è piena". Anche il server nel suo complesso ha un limite al numero di connessioni che accetta contemporaneamente. Questi limiti esistono per mantenere le stanze intime e prevenire abusi.`,
  faq_15_title: "C'è un limite di frequenza?",
  faq_15_body: `Sì. Ogni connessione è limitata a 1 messaggio al secondo. Ogni stanza ha inoltre un limite anti-flood sul numero totale di messaggi che possono essere inviati in un breve intervallo di tempo. Ciò previene lo spam e gli abusi senza richiedere alcuna verifica dell'identità. Se invii messaggi troppo rapidamente, vedrai un breve avviso "Rallenta".

Anche le nuove connessioni sono limitate. Sulla clearnet, il reverse proxy davanti al server limita il numero di connessioni che ogni indirizzo di rete può aprire. Il servizio Tor è protetto dalle difese proof-of-work di Tor per i servizi onion, che rendono costoso sommergerlo di connessioni. Nessuna di queste misure richiede che l'applicazione nullchat riceva o memorizzi il tuo indirizzo IP.`,
...
Entrambe le parti devono concordare di abilitare l'interruttore — funziona allo stesso modo della concordanza del segreto condiviso. L'intestazione della chat mostra "TOR ONLY" in verde quando è attivo, o "CLEARNET" in rosso per le stanze standard, così sai sempre in quale modalità ti trovi.`,
  ...
  faq_19_body: `Sulla rete clearnet (nullchat.org), la pagina web è servita da Vercel e la connessione della chat raggiunge il nostro server su ws.nullchat.org attraverso un reverse proxy nginx. Come per qualsiasi sito web, l'host della pagina e il reverse proxy vedono necessariamente l'indirizzo IP da cui ti connetti nel momento in cui ti connetti. nginx non inoltra il tuo indirizzo all'applicazione nullchat e non lo registra, quindi l'applicazione non riceve né memorizza mai gli indirizzi IP dei client. Se non vuoi che l'host della pagina o il nostro server vedano il tuo indirizzo IP, usa Tor.
...
La lingua che scegli viene conservata in sessionStorage solo per la scheda corrente e viene cancellata quando chiudi la scheda.`,
