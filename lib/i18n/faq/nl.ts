...
Dit is handig als iemand over je schouder meekijkt of als je scherm zichtbaar is voor anderen. Op het eerste gezicht lijkt het alsof je een document bewerkt, niet dat je een versleuteld gesprek voert. Druk nogmaals vijf keer op Shift om terug te keren naar de normale chatweergave.`,
  ...
  faq_32_title: "Kan ik nullchat aan mijn startscherm toevoegen?",
  faq_32_body: `Ja. nullchat ondersteunt Toevoegen aan startscherm op zowel iOS als Android. Tik in iOS Safari op de deelknop en selecteer "Zet op beginscherm". Tik in Android Chrome op het menu en selecteer "Toevoegen aan startscherm" of "App installeren". Dit maakt een zelfstandige snelkoppeling die nullchat opent zonder browserchromen — geen adresbalk, geen tabbladen. Het ziet eruit en voelt aan als een native app.

Belangrijk: nullchat maakt bewust geen gebruik van een service worker en slaat geen gegevens offline op. Er is geen offline modus. De startschermsnelkoppeling opent simpelweg de live site — er wordt niets op je apparaat opgeslagen behalve de snelkoppeling zelf. Dit is een beveiligingsbeslissing: het cachen van versleutelde chatpagina's of service worker-scripts op het apparaat zou forensisch bewijs creëren dat nullchat is gebruikt. De snelkoppeling laat geen spoor achter behalve het eigen pictogram, dat op elk moment kan worden verwijderd.`,
};
