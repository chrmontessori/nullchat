...
Die Stärkeanzeige auf dem Eingabebildschirm gibt dir einen ungefähren Eindruck, wie widerstandsfähig dein Geheimnis gegen Brute-Force-Angriffe ist, aber keine Anzeige ersetzt gutes Urteilsvermögen. Im Zweifel: länger und zufälliger.`,
  ...
  faq_4_body: `Wenn du dein gemeinsames Geheimnis eingibst, verarbeitet dein Browser es mit Argon2id — einer speicherintensiven Schlüsselableitungsfunktion — in zwei getrennten Ableitungen. Jede hat ihr eigenes Salt und verwendet 16 MiB Speicher und 3 Iterationen. All das geschieht auf deinem Gerät.

1. Die erste Ableitung erzeugt die Raum-ID. Sie wird an den Server gesendet, damit er weiß, mit welchem Raum er dich verbinden soll. Der Server sieht niemals dein eigentliches Geheimnis.

2. Die zweite Ableitung erzeugt in einem einzigen Durchlauf 64 Bytes, die in zwei Teile aufgeteilt werden: einen 256-Bit-Verschlüsselungsschlüssel und ein Raumzugangsgeheimnis. Der Verschlüsselungsschlüssel verlässt niemals deinen Browser. Mit dem Raumzugangsgeheimnis beweist dein Browser dem Server, dass du das gemeinsame Geheimnis kennst. Deshalb reicht es nicht, nur die Raum-ID zu kennen, um einem Raum beizutreten.

Da Argon2id für jeden Rateversuch einen großen Block RAM benötigt, ist das Erraten deines Passworts per Brute-Force mit GPU- und ASIC-Hardware weit schwieriger als bei älteren Schlüsselableitungsfunktionen.

Jede Nachricht wird mit NaCl secretbox (XSalsa20-Poly1305) und dem Verschlüsselungsschlüssel verschlüsselt, bevor sie dein Gerät verlässt. Der Server empfängt, speichert und leitet nur Chiffretext weiter, der ohne den Schlüssel bedeutungslos ist. Wir können deine Nachrichten nicht lesen, und niemand kann es, der das gemeinsame Geheimnis nicht kennt.`,
...
• Einen mit Argon2id aus deinem gemeinsamen Geheimnis abgeleiteten Wert, der beweist, dass du es kennst. Der Server speichert davon nur einen Hash, und der Wert verrät weder das Geheimnis noch den Verschlüsselungsschlüssel.
...
Der Absender kann sich jederzeit sicher erneut verbinden, um zu prüfen, ob seine Nachricht noch wartet — ohne einen Countdown auszulösen, solange er der Einzige im Raum ist. Keine der beiden Parteien muss gleichzeitig online sein. Keine der beiden Parteien braucht ein Konto. Keine der beiden Parteien ist identifizierbar. Der Server weiß nie, wer die Nachricht hinterlassen hat oder wer sie abgeholt hat — nur dass ein verschlüsselter Blob gespeichert und später abgerufen wurde. Nach der Löschung gibt es keinen Beweis, dass der Austausch jemals stattgefunden hat.`,
  ...
  faq_10_body: `Ein Raum existiert, solange er aktive Verbindungen oder nicht abgelaufene Nachrichten hat. Sobald die letzte Person die Verbindung trennt und alle Nachrichten abgelaufen oder gelöscht sind, ist der Raum verschwunden. Nichts davon wird aufbewahrt. Wenn nie Nachrichten gesendet werden, ist der Raum nur eine Live-Verbindung — nichts wird gespeichert, und er verschwindet in dem Moment, in dem alle gehen.`,
...
Ein Alias ist eine Bezeichnung, keine verifizierte Identität. Jeder, der das gemeinsame Geheimnis kennt, kann dem Raum beitreten und sich einen beliebigen Alias geben. Behandle deshalb alle in einem Raum als Personen, die das Geheimnis haben. Wenn du sicher sein musst, mit wem du sprichst, bestätige es über einen anderen Kanal, zum Beispiel mit einem vorab vereinbarten Codewort. Teile ein Geheimnis nur mit Menschen, denen du vertraust.`,
  faq_14_title: "Gibt es ein Teilnehmerlimit?",
  faq_14_body: `Jeder Raum unterstützt bis zu 50 gleichzeitige Verbindungen. Wenn der Raum voll ist, siehst du eine „Raum ist voll"-Meldung. Auch der Server insgesamt begrenzt, wie viele Verbindungen er gleichzeitig annimmt. Diese Limits existieren, um Räume überschaubar zu halten und Missbrauch zu verhindern.`,
  faq_15_title: "Gibt es eine Ratenbegrenzung?",
  faq_15_body: `Ja. Jede Verbindung ist auf 1 Nachricht pro Sekunde begrenzt. Jeder Raum hat außerdem ein Flutlimit dafür, wie viele Nachrichten insgesamt in kurzer Zeit gesendet werden können. Dies verhindert Spam und Missbrauch, ohne eine Identitätsverifizierung zu erfordern. Wenn du zu schnell Nachrichten sendest, siehst du einen kurzen „Langsamer"-Hinweis.

Auch neue Verbindungen sind begrenzt. Im Clearnet begrenzt der Reverse-Proxy vor dem Server, wie viele Verbindungen jede Netzwerkadresse öffnen kann. Der Tor-Dienst ist durch die Proof-of-Work-Schutzmechanismen von Tor für Onion-Dienste geschützt, die es teuer machen, ihn mit Verbindungen zu überfluten. Keine dieser Maßnahmen erfordert, dass die nullchat-Anwendung deine IP-Adresse erhält oder speichert.`,
...
Beide Parteien müssen sich auf die Aktivierung des Schalters einigen — es funktioniert genauso wie die Vereinbarung des gemeinsamen Geheimnisses. Der Chat-Header zeigt „TOR ONLY" in Grün an, wenn aktiv, oder „CLEARNET" in Rot für Standardräume, sodass du immer weißt, in welchem Modus du dich befindest.`,
  ...
  faq_19_body: `Im Clearnet (nullchat.org) wird die Webseite von Vercel ausgeliefert, und die Chat-Verbindung läuft über einen nginx-Reverse-Proxy zu unserem Server unter ws.nullchat.org. Wie bei jeder Website sehen der Hoster der Seite und der Reverse-Proxy im Moment der Verbindung zwangsläufig die IP-Adresse, von der aus du dich verbindest. nginx gibt deine Adresse nicht an die nullchat-Anwendung weiter und protokolliert sie nicht, sodass die Anwendung niemals Client-IP-Adressen erhält oder speichert. Wenn du nicht möchtest, dass der Hoster der Seite oder unser Server deine IP-Adresse sieht, nutze Tor.
...
Deine Sprachauswahl wird nur für den aktuellen Tab in sessionStorage gespeichert und gelöscht, sobald du den Tab schließt.`,
