...
Isso é útil se alguém estiver olhando por cima do seu ombro ou se sua tela estiver visível para outros. À primeira vista, parece que você está editando um documento, não tendo uma conversa criptografada. Pressione Shift cinco vezes novamente para retornar à visualização normal do chat.`,
  ...
  faq_32_title: "Posso adicionar o nullchat à tela inicial?",
  faq_32_body: `Sim. O nullchat suporta Adicionar à Tela Inicial tanto no iOS quanto no Android. No iOS Safari, toque no botão de compartilhamento e selecione "Adicionar à Tela de Início". No Android Chrome, toque no menu e selecione "Adicionar à tela inicial" ou "Instalar app". Isso cria um atalho independente que abre o nullchat sem a interface do navegador — sem barra de endereço, sem abas. Parece e funciona como um aplicativo nativo.

Importante: o nullchat deliberadamente não usa service worker nem armazena dados em cache offline. Não há modo offline. O atalho da tela inicial simplesmente abre o site ao vivo — nada é armazenado no seu dispositivo além do próprio atalho. Esta é uma decisão de segurança: armazenar páginas de chat criptografadas ou scripts de service worker em cache no dispositivo criaria evidência forense de que o nullchat foi usado. O atalho não deixa rastros além do seu próprio ícone, que pode ser excluído a qualquer momento.`,
};
