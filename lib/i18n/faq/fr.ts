...
C'est utile si quelqu'un regarde par-dessus votre épaule ou si votre écran est visible par d'autres. Au premier coup d'oeil, on dirait que vous éditez un document, pas que vous avez une conversation chiffrée. Appuyez cinq fois à nouveau sur Maj pour revenir à l'affichage normal du chat.`,
  ...
  faq_32_title: "Puis-je ajouter nullchat à mon écran d'accueil ?",
  faq_32_body: `Oui. nullchat prend en charge l'ajout à l'écran d'accueil sur iOS et Android. Sur iOS Safari, appuyez sur le bouton de partage et sélectionnez « Sur l'écran d'accueil ». Sur Android Chrome, appuyez sur le menu et sélectionnez « Ajouter à l'écran d'accueil » ou « Installer l'appli ». Cela crée un raccourci autonome qui ouvre nullchat sans l'interface du navigateur — pas de barre d'adresse, pas d'onglets. L'apparence et le comportement sont ceux d'une application native.

Important : nullchat n'utilise délibérément aucun service worker et ne met aucune donnée en cache hors ligne. Il n'y a pas de mode hors ligne. Le raccourci de l'écran d'accueil ouvre simplement le site en direct — rien n'est stocké sur votre appareil au-delà du raccourci lui-même. C'est une décision de sécurité : mettre en cache des pages de chat chiffrées ou des scripts de service worker sur l'appareil créerait des preuves forensiques que nullchat a été utilisé. Le raccourci ne laisse aucune trace au-delà de sa propre icône, qui peut être supprimée à tout moment.`,
};
