...
export const fr: Record<FaqKey, string> = {
  ...
  faq_2_body: `Vous et la personne avec qui vous souhaitez communiquer convenez d'un secret partagé à l'avance — en personne, par téléphone, comme vous voulez. Vous entrez tous les deux ce secret dans nullchat et vous vous retrouvez dans le même salon chiffré. Il n'y a pas de liste de salons, pas d'annuaire, aucun moyen de parcourir. Pour entrer, votre navigateur doit prouver au serveur qu'il connaît le secret : connaître l'identifiant d'un salon ne suffit donc pas à lui seul. Si vous ne connaissez pas le secret, le salon n'existe pas pour vous.`,
...
L'indicateur de robustesse sur l'écran d'entrée vous donne une idée approximative de la résistance de votre secret aux attaques par force brute, mais aucun indicateur ne remplace le bon sens. En cas de doute, faites-le plus long et plus aléatoire.`,
  ...
  faq_4_body: `Lorsque vous saisissez votre secret partagé, votre navigateur le fait passer par Argon2id — une fonction de dérivation de clés à forte consommation de mémoire — au cours de deux dérivations distinctes. Chacune possède son propre sel et utilise 16 MiB de mémoire et 3 itérations. Tout cela se passe sur votre appareil.

1. La première dérivation produit l'identifiant du salon. Il est envoyé au serveur pour que celui-ci sache à quel salon vous connecter. Le serveur ne voit jamais votre véritable secret.

2. La seconde dérivation produit 64 octets en une seule passe, divisés en deux : une clé de chiffrement de 256 bits et un secret d'accès au salon. La clé de chiffrement ne quitte jamais votre navigateur. Le secret d'accès au salon permet à votre navigateur de prouver au serveur que vous connaissez le secret partagé ; c'est pourquoi connaître uniquement l'identifiant du salon ne suffit pas pour le rejoindre.

Comme Argon2id exige un gros bloc de RAM pour chaque tentative, il rend la recherche de votre mot de passe par force brute sur GPU et ASIC bien plus difficile qu'avec les anciennes fonctions de dérivation de clés.

Chaque message est chiffré avec NaCl secretbox (XSalsa20-Poly1305) à l'aide de la clé de chiffrement avant de quitter votre appareil. Le serveur reçoit, stocke et relaie uniquement du texte chiffré, qui n'a aucun sens sans la clé. Nous ne pouvons pas lire vos messages, et personne ne le peut sans connaître le secret partagé.`,
...
• Une valeur dérivée de votre secret partagé avec Argon2id, qui prouve que vous le connaissez. Le serveur n'en conserve qu'un hash, et cette valeur ne révèle ni le secret ni la clé de chiffrement.
...
L'expéditeur peut se reconnecter à tout moment pour vérifier si son message est toujours en attente — sans déclencher de compte à rebours, tant qu'il est le seul dans le salon. Aucune des deux parties n'a besoin d'être en ligne en même temps. Aucune des deux parties n'a besoin d'un compte. Aucune des deux parties n'est identifiable. Le serveur ne sait jamais qui a déposé le message ni qui l'a récupéré — seulement qu'un bloc chiffré a été stocké puis récupéré. Après la destruction, il n'existe aucune preuve que l'échange a eu lieu.`,
  ...
  faq_10_body: `Un salon existe tant qu'il a des connexions actives ou des messages non expirés. Une fois que la dernière personne se déconnecte et que tous les messages ont expiré ou été détruits, le salon disparaît. Rien n'en est conservé. Si aucun message n'est jamais envoyé, le salon n'est qu'une connexion en direct — rien n'est stocké, et il disparaît dès que tout le monde part.`,
...
Un alias est une étiquette, pas une identité vérifiée. Toute personne qui connaît le secret partagé peut rejoindre le salon et choisir l'alias qu'elle veut ; considérez donc chaque personne présente dans un salon comme quelqu'un qui détient le secret. Si vous devez être sûr de l'identité de votre interlocuteur, vérifiez-la par un autre canal, par exemple en convenant à l'avance d'un mot de code. Ne partagez un secret qu'avec des personnes de confiance.`,
  faq_14_title: "Y a-t-il une limite de participants ?",
  faq_14_body: `Chaque salon prend en charge jusqu'à 50 connexions simultanées. Si le salon est plein, vous verrez le message « Le salon est plein ». Le serveur dans son ensemble limite aussi le nombre de connexions qu'il accepte en même temps. Ces limites existent pour garder les salons intimistes et prévenir les abus.`,
  faq_15_title: "Y a-t-il une limitation de débit ?",
  faq_15_body: `Oui. Chaque connexion est limitée à 1 message par seconde. Chaque salon a aussi une limite anti-inondation sur le nombre total de messages pouvant être envoyés sur une courte période. Cela empêche le spam et les abus sans nécessiter de vérification d'identité. Si vous envoyez des messages trop rapidement, vous verrez brièvement la mention « Ralentissez ».

Les nouvelles connexions sont limitées elles aussi. Sur le clearnet, le proxy inverse placé devant le serveur limite le nombre de connexions que chaque adresse réseau peut ouvrir. Le service Tor est protégé par les défenses par preuve de travail de Tor pour les services onion, qui rendent coûteux le fait de l'inonder de connexions. Aucune de ces mesures n'exige que l'application nullchat reçoive ou stocke votre adresse IP.`,
...
Les deux parties doivent convenir d'activer l'interrupteur — cela fonctionne de la même manière que pour convenir du secret partagé. L'en-tête du chat affiche « TOR UNIQUEMENT » en vert lorsqu'il est actif, ou « CLEARNET » en rouge pour les salons standards, afin que vous sachiez toujours dans quel mode vous êtes.`,
  ...
  faq_19_body: `Sur le clearnet (nullchat.org), la page web est servie par Vercel, et la connexion du chat passe par un proxy inverse nginx jusqu'à notre serveur, à l'adresse ws.nullchat.org. Comme pour tout site web, l'hébergeur de la page et le proxy inverse voient nécessairement l'adresse IP depuis laquelle vous vous connectez, au moment de la connexion. nginx ne transmet pas votre adresse à l'application nullchat et ne la journalise pas : l'application ne reçoit et ne stocke donc jamais les adresses IP des clients. Si vous ne voulez pas que l'hébergeur de la page ou notre serveur voie votre adresse IP, utilisez Tor.
...
Votre choix de langue est conservé dans sessionStorage pour l'onglet en cours uniquement, et il est effacé lorsque vous fermez l'onglet.`,
...
Ce sont des protections basées sur la friction, pas des garanties absolues. Un utilisateur déterminé peut toujours photographier son écran avec un autre appareil ou utiliser des outils au niveau du système d'exploitation qui contournent les restrictions du navigateur. L'objectif est de rendre la capture occasionnelle difficile et de renforcer l'idée que les conversations dans nullchat ne sont pas destinées à être conservées.`,
  ...
  faq_23_body: `nullchat envoie automatiquement des messages factices chiffrés à intervalles aléatoires (toutes les 10 à 60 secondes) pendant que vous êtes connecté à un salon. Ces messages leurres sont indiscernables des vrais messages — ils ont la même taille (grâce au remplissage fixe), sont chiffrés avec la même clé, relayés par le même chemin serveur, et produisent entre votre navigateur et le serveur la même séquence de trames qu'un vrai message. Le client du destinataire les ignore silencieusement après déchiffrement.
